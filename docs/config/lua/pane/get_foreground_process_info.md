...
* This information is available for local panes and is transported from a compatible mux server to multiplexer client panes. Unix mux clients check foreground identity at least every five seconds and refresh cached process details when its path or process id changes. Other mux domains follow their normal connection polling cadence. A plain `ssh` session cannot report the process running on the remote host.
