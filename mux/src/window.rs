...
impl Window {
    ...
    pub fn move_tab_by_id(&mut self, source: TabId, target: TabId, before: bool) -> bool {
        let Some(source_index) = self.idx_by_id(source) else {
            return false;
        };
        let Some(target_index) = self.idx_by_id(target) else {
            return false;
        };
        ...
        if let Some(active) = active.and_then(|tab_id| self.idx_by_id(tab_id)) {
            self.active = active;
        }
        self.invalidate();
        true
